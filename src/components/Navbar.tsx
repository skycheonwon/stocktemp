      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="text-xl font-bold tracking-tight bg-gradient-to-r from-slate-100 to-slate-300 bg-clip-text text-transparent">
            StockTemp <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-800 text-slate-300 border border-slate-700 ml-1.5 align-middle">{t('stocktemp')}</span>
          </span>
        </Link>

        {/* Right Section: Nav Links + Language Selector */}
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <Link
              to="/watchlist"
              className={`text-sm font-medium transition-colors hover:text-white ${
                isActive('/watchlist') ? 'text-blue-400' : 'text-slate-400'
              }`}
            >
              {t('watchlist')}
            </Link>
